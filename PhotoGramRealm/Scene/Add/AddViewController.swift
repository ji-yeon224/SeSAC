...
class AddViewController: BaseViewController {
    ...
    @objc func saveButtonClicked() {
        ...
        let task = DiaryTable(diaryTitle: titleTextField.text ?? "제목없음", diaryDate: Date(), diaryContent: contentTextView.text, diaryPhoto: fullURL)
        //print(realm.configuration.fileURL!)
        // 입력 값에 대한 조건 처리 후 write하기 (글자수 제한 등)
        
        repository.createItem(task)
        
        if userImageView.image != nil {
            saveImageToDocument(fileName: "jy_\(task._id).jpg", image: userImageView.image!)
        }
